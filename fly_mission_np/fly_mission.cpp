...
int main(int argc, char** argv)
{
    ...
    std::ifstream ipx;
    ipx.open("/home/doannd/MAVSDK_Project_Test/data/altsp");
    ...
    std::ifstream ip;
    ip.open("/home/doannd/MAVSDK_Project_Test/data/input");
    ...
    const Telemetry::Result set_rate_result_battery = telemetry.set_rate_battery(0.25);
        ...
        std::cout << ERROR_CONSOLE_TEXT << "Setting rate failed:" << set_rate_result_battery
                  << NORMAL_CONSOLE_TEXT << std::endl;
        return 1;
    }
        ...
        std::cout << "Possion:" << std::endl
                  ...
                  << std::endl
                  << "Latitude: "<< std::setprecision(5) << position.latitude_deg << " degree"
                  << std::endl
                  << "Longitude: "<< std::setprecision(5) << position.longitude_deg << " degree"
                  << NORMAL_CONSOLE_TEXT // set to default color again
                  << std::endl;

        myfile.open("/home/doannd/MAVSDK_Project_Test/data/output",std::ios::app);
        myfile << position.relative_altitude_m << ",";
        myfile << std::fixed << std::setprecision(5) << position.latitude_deg << ",";
        myfile << std::fixed << std::setprecision(5) << position.longitude_deg << ",";
        myfile.close();       
    });
    
    	std::atomic<bool> is_stop{false};
    	// get battery percent
    	telemetry.subscribe_battery([&myfile, &action, &is_stop, &mission](Telemetry::Battery battery) {
    	std::cout << std::fixed;
        ...
        myfile.open("/home/doannd/MAVSDK_Project_Test/data/output",std::ios::app);
        myfile << std::fixed << std::setprecision(2) << battery.remaining_percent << "\n";
        myfile.close(); 
        
             
  /*  	std::ifstream ipconfig;
    	ipconfig.open("/home/doannd/MAVSDK_Project_Test/data/config");
	if(ipconfig.good()){
	   std::string stop;
	   getline(ipconfig, stop);
		if(stop != ""){ 	
		is_stop = true;
    		ipconfig.close(); 
        ...
        std::cout << "clear mission..." << std::endl;
        ...
        if (result != Mission::Result::Success) {
    		...
    		const Action::Result result1 = action.land();

        	if (result1 != Action::Result::Success) {
            		std::cout << "Failed to command RTL (" << result1 << ")" << std::endl;
        	} else {
